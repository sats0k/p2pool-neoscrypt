...
class Node(object):
    def __init__(self, factory, daemon, shares, known_verified_share_hashes, net):
    ...
    def start(self):
        ...
        self.daemon_work = variable.Variable((yield helper.getwork(self.daemon)))
        @defer.inlineCallbacks
        def work_poller():
            while stop_signal.times == 0:
                flag = self.factory.new_block.get_deferred()
                try:
                    self.daemon_work.set((yield helper.getwork(self.daemon, self.daemon_work.value['use_getblocktemplate'])))
                except:
                    log.err()
        ...
        def handle_header(new_header):
            ...
            if not (self.net.PARENT.POW_FUNC(bitcoin_data.block_header_type.pack(new_header)) <= self.daemon_work.value['bits'].target):
                return
            daemon_best_block = self.daemon_work.value['previous_block']
        ...
        self.daemon_work.changed.watch(lambda _: self.set_best_share())
        self.set_best_share()
        
        # setup p2p logic and join p2pool network
        
        # update mining_txs according to getwork results
        @self.daemon_work.changed.run_and_watch
        def _(_=None):
            new_mining_txs = {}
            new_known_txs = dict(self.known_txs_var.value)
            for tx_hash, tx in zip(self.daemon_work.value['transaction_hashes'], self.daemon_work.value['transactions']):
        ...
        def _(tx):
            ...
            self.known_txs_var.set(new_known_txs)
        # forward transactions seen to daemon
        ...
        def _(share):
            ...
            if block is None:
                print('GOT AN INCOMPLETE BLOCK FROM A PEER! %s bitcoin: %s%064x' % (p2pool_data.format_hash(share.hash), self.net.PARENT.BLOCK_EXPLORER_URL_PREFIX, share.header_hash), file=sys.stderr)
                return
            helper.submit_block(block, True, self.factory, self.daemon, self.daemon_work, self.net)
            print()
            print('GOT A BLOCK FROM A PEER! Passing to the daemon! %s bitcoin: %s%064x' % (p2pool_data.format_hash(share.hash), self.net.PARENT.BLOCK_EXPLORER_URL_PREFIX, share.header_hash))
            print()
    ...
    def set_best_share(self):
        best, desired, decorated_heads, bad_peer_addresses = self.tracker.think(self.get_height_rel_highest, self.daemon_work.value['previous_block'], self.daemon_work.value['bits'], self.known_txs_var.value)
